...
export default function ChartOfAccountEditEditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/finance/accounts/${recordId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/finance/accounts/${recordId}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred');
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {["accountCode","accountName","accountType","parentId","isActive"].map((field) => (
                  <div key={field}>
