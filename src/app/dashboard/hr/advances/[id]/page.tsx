...
export default function EmployeeAdvanceDetailDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/hr/advances/${recordId}`);
  ...
  const handleEdit = () => {
    router.push(`/dashboard/hr/advances/${recordId}/edit`);
  };
        ...
        <CardBody className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {["employeeId","amount","remainingBal","monthlyDeduction","reason","status","issueDate"].map((field) => (
                <div key={field}>
