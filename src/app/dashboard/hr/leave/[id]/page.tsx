...
export default function LeaveRequestDetailDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/hr/leave/${recordId}`);
  ...
  const handleEdit = () => {
    router.push(`/dashboard/hr/leave/${recordId}/edit`);
  };
        ...
        <CardBody className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {["employeeId","leaveType","startDate","endDate","days","reason","status"].map((field) => (
                <div key={field}>
