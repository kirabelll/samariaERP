...
const reports: ReportCard[] = [
  {
    ...
    description: 'Invoices by age (0-30, 31-60, 61-90, 90+ days)',
    icon: '📅',
  },
  {
    id: 'payable-aging',
    title: 'Payable Aging Report',
    description: 'Supplier payables by age brackets',
    icon: '💳',
  },
  {
    id: 'unresolved-exceptions',
    title: 'Unresolved Exceptions',
    description: 'Grouped by exception type and status',
    icon: '⚠️',
  },
  {
    id: 'document-verification',
    title: 'Document Verification Status',
    description: 'Missing and unverified documents',
    icon: '📄',
  },
  {
    id: 'approval-turnaround',
    title: 'Approval Turnaround Report',
    description: 'Average approval time by module',
    icon: '✅',
  },
];
...
export default function ExceptionReportsPage() {
  ...
  const fetchReportData = async (reportId: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/exception?${params}`);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/exception/export?${params}`);
