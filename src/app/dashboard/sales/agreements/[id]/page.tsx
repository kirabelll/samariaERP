...
export default function SalesAgreementDetailPage() {
  ...
  const [data, setData] = useState<SalesAgreementData | null>(null);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/sales/agreements/${recordId}`);
  ...
  const handleEdit = () => {
    router.push(`/dashboard/sales/agreements/${recordId}/edit`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600 mt-1">Sales Agreement Details</p>
      </div>
      ...
      <Card>
        ...
        <CardBody className="space-y-8">
          ...
          {parsedItems.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Agreement Items</h3>
