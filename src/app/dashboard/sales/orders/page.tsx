    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <Link href="/dashboard/sales/orders/new">
          <Button variant="primary" size="lg">+ New Order</Button>
        </Link>
      </div>
      ...
      <Card>
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Input placeholder="Search by order no or customer..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
            <Select
              options={[
          ...
          <div className="mt-4 text-sm text-gray-600">
            {loading ? 'Loading...' : `Showing ${data.length} of ${pagination.total} orders`}
          </div>
        </CardBody>
      </Card>
      ...
      <Card>
        <CardBody>
          ...
          <Table<SalesOrder>
            ...
            emptyMessage={loading ? 'Loading...' : 'No sales orders found'}
          />
        </CardBody>
      </Card>
    </div>
