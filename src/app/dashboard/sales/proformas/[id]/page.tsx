...
export default function ProformaDetailDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/sales/proformas/${recordId}`);
  ...
  const handleEdit = () => {
    router.push(`/dashboard/sales/proformas/${recordId}/edit`);
  };
        ...
        <CardBody className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {["proformaNo","customerId","division","subtotal","vatAmount","withholding","totalAmount","validityDays","terms","status"].map((field) => (
                <div key={field}>
